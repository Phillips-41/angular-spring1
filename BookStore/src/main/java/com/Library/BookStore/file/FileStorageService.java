...
public class FileStorageService {
    ...
    public String saveFile(@NotNull MultipartFile sourceFile,
        ...
        final String finalUploadSubPath = "users" + separator + userId;
    ...
    private String uploadFile(@NotNull MultipartFile sourceFile,@NotNull String finalUploadSubPath) {
        final String finalUploadPath = fileUploadPath + separator + finalUploadSubPath;
        File targetFolder = new File(fileUploadPath);

        if (!targetFolder.exists()) {
           boolean folderCreated = targetFolder.mkdirs();
           if(!folderCreated){
               log.warn("folder is not created");
           }
        }
